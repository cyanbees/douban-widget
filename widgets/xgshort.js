...
WidgetMetadata = {
  ...
  modules: [
    ...
    },
    {
      ...
      params: [
        {
          name: "channelId", title: "频道", type: "enumeration",
          enumOptions: [
            { title: "短剧", value: "1" },
            { title: "电影", value: "2" },
            { title: "电视剧", value: "3" }
        ...
        {
          name: "sort", title: "排序", type: "enumeration",
          enumOptions: [
            { title: "最新", value: "0" },
            { title: "人气最高", value: "1" },
            { title: "评分最高", value: "2" }
...
function feedEpisodeItem(entry) {
  ...
  var versionId = makeVersionId(entry);
  return {
...
function episodeItem(episode, series, cover) {
  ...
  return {
    ...
    action: {
      ...
      title: episodeTitle,
      url: firstPlayableURL(episode) || makeVersionId(episode),
...
async function loadCategoryList(params) {
  try {
    ...
    var sortValue = stringValue(params.sort) || '0';
    var page = positiveInt(params.page, 1);
...
async function searchShort(params) {
  try {
    ...
    return {
      pageType: 'category',
      id: 'xgshort-search:' + encodeURIComponent(query),
      title: '搜索 ' + query,
      style: 'media.posterGrid',
      itemAspectRatio: '2:3',
      items: items,
...
async function loadResource(params) {
  try {
    ...
    if (directURL && /^https?:\/\//i.test(directURL) && directURL.indexOf('xgplay:') !== 0) {
      return [{
        name: '播放',
        url: directURL,
    ...
    // 从 versionId 解析 accessKey
    var accessKey = '';
    if (versionId.indexOf('xgplay:') === 0) {
      accessKey = decodeURIComponent(versionId.split(':')[1] || '');
    }

    // 尝试从 episodeId 或系列信息获取
    if (!accessKey) {
      var episodeId = stringValue(params.episodeId);
      if (episodeId) {
        // 尝试从缓存详情中获取 episode 的 accessKey
        var seriesShortId = parseSeriesShortId(stringValue(params.link || params.itemId));
        if (seriesShortId) {
          var cacheKey = 'xgshort.detail.' + seriesShortId;
          var cached = storageGet(cacheKey);
          if (cached && cached.data) {
            ...
            var ep = episodes.find(function (e) {
              return String(e.shortId || e.episodeShortId || e.id) === episodeId ||
                     String(e.episodeNumber) === episodeId;
            });
